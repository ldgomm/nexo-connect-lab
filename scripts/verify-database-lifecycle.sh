...
if [[ "$(query_scalar "SELECT count(*) FROM public.flyway_schema_history WHERE version IN ('1', '2', '3', '4', '5', '6') AND success")" != "6" ]] ||
...
if [[ "$(query_scalar "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'connect' AND has_table_privilege('${APP_USER}', quote_ident(table_schema) || '.' || quote_ident(table_name), 'SELECT') AND has_table_privilege('${APP_USER}', quote_ident(table_schema) || '.' || quote_ident(table_name), 'INSERT') AND has_table_privilege('${APP_USER}', quote_ident(table_schema) || '.' || quote_ident(table_name), 'UPDATE')")" != "7" ]] ||
