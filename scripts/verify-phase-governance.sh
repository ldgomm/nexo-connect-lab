...
require_property "$POLICY_FILE" accepted.phase CONNECT.12
require_property "$POLICY_FILE" accepted.head 655d733b8668011815f66e065d42c5d66c9e555e
require_property "$POLICY_FILE" establishing.phase CONNECT.13
require_property "$POLICY_FILE" next.phase CONNECT.14
...
expected_phases="CONNECT.B CONNECT.C1 CONNECT.C2 CONNECT.C3 CONNECT.C4 CONNECT.C5 CONNECT.C6 CONNECT.07 CONNECT.USER.BASELINE CONNECT.08 CONNECT.09 CONNECT.10 CONNECT.11 CONNECT.12"
actual_phases=""
baseline_count=0
current_count=0
next_count=0
...
while IFS=$'\t' read -r record phase status commit subject; do
    ...
    case "$record" in
        ...
        CURRENT)
            ...
            [[ "$phase" == "CONNECT.13" && "$status" == "IMPLEMENTING" && "$commit" == "DISCOVER_BY_SUBJECT" ]] ||
                fail "LEDGER_CURRENT_MISMATCH"
            current_subject="$subject"
            ;;
        NEXT)
            next_count=$((next_count + 1))
            [[ "$phase" == "CONNECT.14" && "$status" == "LOCKED" && "$commit" == "-" ]] ||
...
[[ "$baseline_count" -eq 14 && "$actual_phases" == "$expected_phases" ]] ||
    fail "LEDGER_BASELINE_SEQUENCE_MISMATCH"
[[ "$current_count" -eq 1 && "$next_count" -eq 1 ]] || fail "LEDGER_PHASE_CARDINALITY_MISMATCH"

connect_13_matches="$(git log HEAD --format='%H%x09%s' | awk -F '\t' -v expected="$current_subject" '$2 == expected { print $1 }')"
connect_13_count="$(printf '%s\n' "$connect_13_matches" | awk 'NF { count++ } END { print count + 0 }')"

if [[ "$connect_13_count" == "0" ]]; then
    [[ "$(git rev-parse HEAD)" == "655d733b8668011815f66e065d42c5d66c9e555e" ]] ||
        fail "CONNECT_13_COMMIT_MISSING_AFTER_BASELINE"
elif [[ "$connect_13_count" == "1" ]]; then
    connect_13_commit="$connect_13_matches"
    [[ "$(git rev-parse "${connect_13_commit}^")" == "655d733b8668011815f66e065d42c5d66c9e555e" ]] ||
        fail "CONNECT_13_PARENT_MISMATCH"
    git merge-base --is-ancestor "$connect_13_commit" HEAD || fail "CONNECT_13_NOT_ANCESTOR"
else
    fail "CONNECT_13_COMMIT_NOT_UNIQUE"
fi
...
printf 'ACCEPTED_CONNECT_12=PASS\n'
